...
private:
    void setup()
    {
        ...
        // 加载点云数据
        point_cloud.positions = load_point_cloud_positions("cloud.xyz");
        point_cloud.colors = load_point_cloud_colors("cloud.rgb");
        point_cloud.upload_to_gpu(gl);
        
        // 编译点云着色器程序
        point_cloud_program = compile_shader_program(
...
struct PointCloud {
    ...
    std::vector<float> colors;    // rgb colors
    GLuint vbo_pos;
    GLuint vbo_color;
    
    void upload_to_gpu(const glow::Context &gl) {
        // 上传点云数据到GPU
